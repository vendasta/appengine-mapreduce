...
class _HashingGCSOutputWriter(output_writers.OutputWriter):
  ...
  def write(self, data):
    ...
    proto.key = key
    proto.value = value
    pool.append(proto.SerializeToString())
...
class ShufflePipeline(pipeline_base.PipelineBase):
  ...
  def run(self, job_name, mapper_params, filenames, shards=None):
    ...
    with pipeline.After(merged_files):
      ...
      yield _GCSCleanupPipeline(bucket_name, all_temp_files)
...
class _GCSCleanupPipeline(pipeline_base.PipelineBase):
  ...
  def delete_file_or_list(self, bucket_name, filename_or_list):
    ...
    if isinstance(filename_or_list, list):
      ...
      retry_params = Retry(initial=1.0, maximum=self._MAX_RETRIES)
      # pylint: disable=bare-except
      try:
        blob.delete(retry=retry_params)
      except:
        pass

  def run(self, bucket_name, temp_files):
    self.delete_file_or_list(bucket_name, temp_files)
