...
class HugeTask:
  ...
  def decode_payload(cls, request):
    ...
    return cls._decode_payload(request.data)
  ...
  def _decode_payload(cls, body):
    ...
    result = {}
    for (name, value) in list(urllib.parse.parse_qs(payload_str).items()):
      if len(value) == 1:
        result[name] = value[0]
      else:
        result[name] = value
...
class TransientShardState:
  ...
  def from_request(cls, request):
    ...
    mapreduce_spec = MapreduceSpec.from_json_str(request.form.get("mapreduce_spec", request.args.get("mapreduce_spec")))
    mapper_spec = mapreduce_spec.mapper
    input_reader_spec_dict = json.loads(request.form.get("input_reader_state", request.args.get("input_reader_state")), cls=json_util.JsonDecoder)
    input_reader = mapper_spec.input_reader_class().from_json(input_reader_spec_dict)
    initial_input_reader_spec_dict = json.loads(request.form.get("initial_input_reader_state", request.args.get("initial_input_reader_state")), cls=json_util.JsonDecoder)
    ...
    if mapper_spec.output_writer_class():
      output_writer = mapper_spec.output_writer_class().from_json(
          json.loads(request.form.get("output_writer_state", request.args.get("output_writer_state", "{}")),
    ...
    handler = util.try_deserialize_handler(request.form.get("serialized_handler", request.args.get("serialized_handler")))
    if not handler:
      handler = mapreduce_spec.mapper.handler

    return cls(mapreduce_spec.params["base_path"],
               mapreduce_spec,
               str(request.form.get("shard_id", request.args.get("shard_id"))),
               int(request.form.get("slice_id", request.args.get("slice_id"))),
               input_reader,
               initial_input_reader,
               output_writer=output_writer,
               retries=int(request.form.get("retries", request.args.get("retries"))),
               handler=handler)
